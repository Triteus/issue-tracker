...
const Ticket_1 = __importStar(require("./Ticket"));
...
describe('Ticket model', () => {
    describe('set subTasks', () => {
        it('sets subTasks (method)', async () => {
            ...
            const ticket = new Ticket_1.default(ticket_1.ticketData());
    ...
    it('sets editor and saves (method)', async () => {
        jest.spyOn(Ticket_1.default.prototype, 'save')
    ...
    describe('change status method', () => {
        it('clears path assigend when status switches to open', async () => {
            ...
            const editorId = new mongoose_1.Types.ObjectId();
            ticket.changeStatus(Ticket_1.TicketStatus.OPEN, editorId);
            expect(ticket.status).toBe(Ticket_1.TicketStatus.OPEN);
        ...
        it('clears path assigendTo when status switches to closed', async () => {
            ...
            ticket.assignedTo = new mongoose_1.Types.ObjectId();
            const editorId = new mongoose_1.Types.ObjectId();
            ticket.changeStatus(Ticket_1.TicketStatus.CLOSED, editorId);
            expect(ticket.status).toBe(Ticket_1.TicketStatus.CLOSED);
            expect(ticket.assignedTo).toBeFalsy();
        });
        it('changes path assignedTo to given editorId', async () => {
            ...
            const editorId = new mongoose_1.Types.ObjectId();
            ticket.changeStatus(Ticket_1.TicketStatus.ACTIVE, editorId);
            expect(ticket.status).toBe(Ticket_1.TicketStatus.ACTIVE);
    ...
    describe('toJSON', () => {
        ...
        it('maps from editorHistory.editorId to editorHistory.editor', () => {
            ...
            const ticket = new Ticket_1.default({ ...ticket_1.ticketData(), editorHistory: [history] });
            const ticketJSON = ticket.toJSON();
